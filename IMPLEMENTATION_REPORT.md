...
- `04-evolving-island.md` §8의 자산 계획에 맞춰 기준 장면 1장과 신규 생성 이미지 8장(개체 표현형 4, 서식지 2, 계절 맥락 2)을 앱 갤러리와 개체 카드에 연결.
...
- 배포 전 잔여 검증으로 320/375/768/1280px 네 폭에서 이미지 9장 로딩, 한 세대→20세대→seed+i 10회→관찰·예측·설명 저장 완주, 가로 스크롤 부재, 런타임 오류 0건을 재기록했다. 상세 결과는 [`four-width-device-flow.json`](output/qa/four-width-device-flow.json)에 남겼다.
...
생성 이미지는 내장 image_gen 도구로 제작했다. 호출 인자에 Image 2.5 모델 선택기가 노출되지 않아 특정 이미지 모델 사용을 주장하지 않는다. 9장 전체의 프롬프트·캡션·검수 기록은 [이미지 메타데이터](output/assets/island-assets.metadata.json), 생성 프롬프트 요약은 [프롬프트 기록](output/assets/island-prompts.md)에 있다. 풍경과 개체 이미지는 계산 근거로 사용하지 않는다.
...
최종 캡처 검토에서 창 크기 변경 후 캔버스가 비는 문제를 찾아 `resize()` 직후 재렌더를 추가했다. 원본과 사본을 함께 수정했다. 이번 자산 연결 이후에는 네 폭 완주 기록을 새로 남겨 모바일·태블릿·데스크톱 폭에서 이미지와 기록 흐름을 함께 확인했다.
...
현재 변경은 배포 전 상태이며 기존 공개 Pages는 이전 커밋을 가리킨다. 커밋·푸시·Pages 재배포 후 공개 주소에서 다시 확인해야 한다. 실제 교실 사용 및 생물학 전문가 검수는 수행하지 않았다. VoiceOver 구현·검증, 로그인·영구 저장·점수·음성·런타임 AI는 범위에 포함하지 않았다. HVC 등록·갤러리 동기화는 별도 요청 범위가 아니다.
