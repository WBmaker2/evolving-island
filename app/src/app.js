...
import { ISLAND_ASSETS, ISLAND_BASE_ASSET, ORGANISM_ASSET_BY_PHENOTYPE } from './assets/islandAssets.js';
...
function assetGallery() {
  return `<details class="asset-gallery" data-od-id="asset-gallery"><summary>시각 자산 9장 살펴보기</summary><p class="section-copy">생성 이미지는 가상의 섬·개체·환경을 상상하기 위한 참고 장면입니다. 수치, 빈도, 유전 결과의 근거가 아닙니다.</p><div class="asset-grid">${ISLAND_ASSETS.map((asset) => `<figure class="asset-card"><img src="${escapeHtml(asset.src)}" alt="${escapeHtml(asset.alt)}" loading="lazy" decoding="async"><figcaption><strong>${escapeHtml(asset.title)}</strong><span>${escapeHtml(asset.caption)}</span></figcaption></figure>`).join('')}</div></details>`;
}
